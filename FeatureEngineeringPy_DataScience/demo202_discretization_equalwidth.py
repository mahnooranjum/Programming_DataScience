# -*- coding: utf-8 -*-
"""Demo202_Discretization_EqualWidth.ipynb
...
def equal_width(X_train, X_test, cols, n):
  for col in cols:
    X_train[col], bins = pd.cut(X_train[col], bins = n, retbins=True, precision=3)
    X_test[col] = pd.cut(X_test[col], bins = bins)

equal_width(X_train, X_test, ['Fare'], 10)
